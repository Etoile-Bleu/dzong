...
fn test_merge_iterator_lsn_priority() -> Result<()> {
    ...
    let iter1 =
        Box::new(std::iter::once(Ok(rec1))) as Box<dyn Iterator<Item = Result<SstableRecord>>>;
    ...
    let iter2 =
        Box::new(std::iter::once(Ok(rec2))) as Box<dyn Iterator<Item = Result<SstableRecord>>>;
